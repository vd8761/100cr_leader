...
/* Button - quiet, editorial CTA.
   solid · onDark · outline · ghost (per the design system). */
export function Button({
  ...
  return (
    ...
    >
      ...
      {children}
      {icon && iconRight && (
        <Icon name={icon} size={18} style={{ transform: hover ? 'translateX(3px)' : 'none', transition: 'transform var(--dur) var(--ease)' }} />
      )}
    </Tag>
  );
}
